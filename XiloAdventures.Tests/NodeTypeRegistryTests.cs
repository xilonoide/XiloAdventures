...
public class NodeTypeRegistryTests
{
    ...
    [Fact]
    public void GetNodesForOwnerType_Conversation_ReturnsDialogueNodes()
    {
        var convNodes = NodeTypeRegistry.GetNodesForOwnerType("Conversation").ToList();

        Assert.True(convNodes.Count > 0);
        Assert.Contains(convNodes, n => n.TypeId == "Conversation_Start");
        Assert.Contains(convNodes, n => n.TypeId == "Conversation_NpcSay");
        Assert.Contains(convNodes, n => n.TypeId == "Conversation_PlayerChoice");
        Assert.Contains(convNodes, n => n.TypeId == "Conversation_End");
    }
