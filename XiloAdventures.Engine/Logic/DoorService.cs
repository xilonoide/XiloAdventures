...
namespace XiloAdventures.Engine.Logic;

/// <summary>
/// Lógica de alto nivel para trabajar con puertas y llaves.
/// Sólo depende de listas de Door / KeyDefinition, no del resto del motor.
/// </summary>
public class DoorService
{
    ...
    /// (normalmente almacenadas en tu WorldModel).
    /// </summary>
    ...
    /// <summary>
    /// Devuelve una puerta por id, o null si no existe.
    /// </summary>
    public Door? GetDoor(string doorId)
        => _doors.FirstOrDefault(d => d.Id == doorId);
    ...
    public bool CanOperateFromRoom(Door door, string currentRoomId)
    {
        ...
        var isFromA = door.RoomIdA == currentRoomId;
        var isFromB = door.RoomIdB == currentRoomId;
    ...
    public bool HasRequiredKey(Door door, IEnumerable<string> availableObjectIds)
    {
        ...
        var availableIds = new HashSet<string>(availableObjectIds ?? Enumerable.Empty<string>());
    ...
    /// <summary>
    /// Intenta cerrar la puerta (si está abierta).
    /// </summary>
